...
# 基于 module-lite-more.sh，添加缺失的关键协议
...
# Program options:
# 注意：启用 programs 会增加约 1-2MB 体积
# 如果只需要播放功能，建议保持禁用
# 如果需要 FFmpeg 命令行功能（转码、截图等），可以启用
...
# 如果需要启用命令行工具，注释掉上面的 disable，改用下面的：
# export COMMON_FF_CFG_FLAGS="$COMMON_FF_CFG_FLAGS --enable-programs"
...
# export COMMON_FF_CFG_FLAGS="$COMMON_FF_CFG_FLAGS --disable-avresample"  # 已移除：FFmpeg 7.x 不再支持此选项
